...
import { useEvent } from "@/context/EventContext";   // ✅ ADDED

export default function LiveEventCarousel({ events }) {
  const [index, setIndex] = useState(0);
  const { setSelectedEvent } = useEvent();           // ✅ ADDED
  ...
  return (
    <div className="relative h-[60vh] md:pl-80 w-full flex justify-center items-center overflow-hidden">
      <div className="absolute z-50 h-full w-[320px] left-0 flex-grow pt-8 pl-8 flex flex-col justify-items-start">
      ...
      <div className="relative w-full h-[360px] flex rounded-2xl overflow-hidden justify-center items-center">
        <motion.div
          key={index}
        ...
        >
          {/* ✅ WRAPPED WITH LINK + CONTEXT ONCLICK */}
          <Link
            href={`/events/${currentEvent._id}`}
            onClick={() => setSelectedEvent(currentEvent)}   // <-- IMPORTANT
          >
            <Image
              ...
              layout="fill"
              objectFit="fill"
              className="brightness-60 border-2 border-red-400 rounded-2xl cursor-pointer"
            />
          </Link>
        </motion.div>
      </div>
